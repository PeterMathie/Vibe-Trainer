...
- [ ] **ARCH-01:** Split dense editors into maintainable components/state holders, centralise validation and remove superseded private screens without changing the agreed navigation.
  ...
  ARCH-01f (detailed-set validation): **IN PROGRESS**. Owner: GitHub Copilot session `33304680-6464-408c-b019-2abe27a4f884`. Branch: `pmathie-cicpilot-persist-workout-drafts`. Claimed: 22 September 2026. Scope: extract the duplicated detailed-set string state, validation and entity conversion shared by recovered-entry and saved-set dialogs into one pure state holder, without changing persistence, visuals or navigation. Acceptance: focused unit tests preserve weighted/hold/unilateral/ROM/failure conversion and all existing invalid-input messages; workout logging and entry-draft instrumentation remains green; full validation passes.

  ARCH-01 remains open after five completed safe slices: `WorkoutEditor.kt` still couples compact-entry recovery, timer behavior and two detailed-set forms; `TrackerScreen.kt` still combines list, daily input and field configuration; `EditorViewModel.kt` still owns heterogeneous editor persistence. ARCH-01f addresses only the duplicated detailed-set validation seam.
