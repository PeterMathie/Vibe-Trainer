...
- [ ] **ARCH-01:** Split dense editors into maintainable components/state holders, centralise validation and remove superseded private screens without changing the agreed navigation.
  ...
  ARCH-01g (detailed-set field component): **IN PROGRESS**. Owner: GitHub Copilot session `33304680-6464-408c-b019-2abe27a4f884`. Branch: `pmathie-cicpilot-persist-workout-drafts`. Claimed: 22 September 2026. Scope: extract the duplicated detailed-set controls into one stateless composable driven by `SetDetailsForm`, while retaining recovered-draft persistence and saved-set state ownership in their existing dialogs. Acceptance: labels, order, band/variation selection and error rendering remain unchanged; focused workout logging/draft tests and full validation pass.

  ARCH-01 remains open: `WorkoutEditor.kt` still couples compact-entry recovery and timer behavior; `TrackerScreen.kt` still combines list, daily input and field configuration; `EditorViewModel.kt` still owns heterogeneous editor persistence. Do not combine these remaining seams into a rewrite.
