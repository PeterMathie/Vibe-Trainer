...
- [ ] **DATA-03:** Define and test historical behaviour when variation order, band definitions or habit targets change; some calculations still read live definitions.
  ...
  Acceptance plan: snapshot variation rank and selected band name/width on submitted sets, and snapshot whether each tracker value met the field target at save time. Historical progress/activity must consume snapshots rather than later edits to live definitions. Add a Room v4 → v5 migration that backfills existing rows from the definitions available at upgrade time without losing records, migration coverage, and focused regression tests proving later definition/target edits do not rewrite history. Document the unavoidable limitation that pre-v5 definition values cannot be reconstructed if they were already edited before upgrade.
