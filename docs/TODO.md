...
UI-01d (history navigation and map modes): **IN PROGRESS**. Owner: GitHub Copilot session `33304680-6464-408c-b019-2abe27a4f884`. Branch: `pmathie-cicpilot-persist-workout-drafts`. Claimed: 22 September 2026. Acceptance: exercise API 35 navigation into historical workout days and back to the current view; switch between strength and stretch historical maps; verify the selected date and mode remain semantically clear and backed by the expected seeded history. Fix only concrete interaction or accessibility defects demonstrated by the focused tests. SVG region and selected-outline coverage remains a separate slice.
