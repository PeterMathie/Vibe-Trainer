...
- [ ] **PROGRESS-01:** Add chart axes/date labels, desktop hover, ROM separation by unit, explicit PR presentation and clearer skill-index explanation.
  ...
  Acceptance: graphs show readable date and value axes; pointer hover/focus exposes the same point detail as touch where the platform supports it; ROM series never combine unlike units; PR types are explicit rather than inferred from session score; skill-index copy explains its heuristic variation/hold/reps/assistance basis. Add focused scorer/series tests and emulator interaction evidence.
