...
- [ ] **ARCH-01:** Split dense editors into maintainable components/state holders, centralise validation and remove superseded private screens without changing the agreed navigation.
  ...
  ARCH-01a (superseded private screens): **IN PROGRESS**. Owner: GitHub Copilot session `33304680-6464-408c-b019-2abe27a4f884`. Branch: `pmathie-cicpilot-persist-workout-drafts`. Claimed: 22 September 2026. Scope: remove only the unreachable private `ProgrammeScreen`, `WorkoutScreen`, `ExerciseLogger` and their exclusively used compact-entry helpers from `VibeTrainerApp.kt`; current navigation already uses `ProgrammeEditor` and `WorkoutEditor`. Acceptance: no visual/navigation changes, focused programme/logging/history Compose suites pass, full unit/instrumentation/build validation passes, and no reachable symbol is removed.
