...
- [ ] **ARCH-01:** Split dense editors into maintainable components/state holders, centralise validation and remove superseded private screens without changing the agreed navigation.
  ...
  ARCH-01h (tracker daily-entry component): **IN PROGRESS**. Owner: GitHub Copilot session `33304680-6464-408c-b019-2abe27a4f884`. Branch: `pmathie-cicpilot-persist-workout-drafts`. Claimed: 22 September 2026. Scope: extract tracker daily input, choice/date-time controls and target summary from `TrackerScreen.kt` into a focused component file without moving persistence or changing layout/navigation. Acceptance: choice/date-time/numeric validation and save/clear behavior remain covered by focused habit tests; full validation passes.

  ARCH-01 remains open: `WorkoutEditor.kt` still couples compact-entry recovery and timer behavior; `TrackerScreen.kt` still combines the tracker list with field configuration; `EditorViewModel.kt` still owns heterogeneous editor persistence. Do not combine these remaining seams into a rewrite.
