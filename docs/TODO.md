...
- [ ] **ARCH-01:** Split dense editors into maintainable components/state holders, centralise validation and remove superseded private screens without changing the agreed navigation.
  ...
  ARCH-01e (editor ordering): **IN PROGRESS**. Owner: GitHub Copilot session `33304680-6464-408c-b019-2abe27a4f884`. Branch: `pmathie-cicpilot-persist-workout-drafts`. Claimed: 22 September 2026. Scope: extract the repeated item lookup/bounds/swap algorithm used by programme, day, programme-exercise, tracker-field and variation moves into a pure helper while retaining each operation’s existing filtering, rank offset and Room transaction. Acceptance: unit tests pin up/down/boundary/missing-ID behavior, focused programme and habit ordering tests pass, then the full suite passes.

  Remaining ARCH-01 work after this slice stays open: `WorkoutEditor.kt` still couples compact-entry recovery, timer behavior and two detailed-set forms; `TrackerScreen.kt` still combines list, daily input and field configuration; `EditorViewModel.kt` still owns heterogeneous editor persistence. Continue by claiming one independently testable state/persistence seam at a time rather than combining these into a rewrite.
