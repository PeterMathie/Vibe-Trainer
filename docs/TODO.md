...
- [ ] **ARCH-01:** Split dense editors into maintainable components/state holders, centralise validation and remove superseded private screens without changing the agreed navigation.
  ...
  ARCH-01i (tracker field configuration): **IN PROGRESS**. Owner: GitHub Copilot session `33304680-6464-408c-b019-2abe27a4f884`. Branch: `pmathie-cicpilot-persist-workout-drafts`. Claimed: 22 September 2026. Scope: extract tracker-field form validation/entity conversion and its dialog from `TrackerScreen.kt`, while leaving list state, persistence calls, labels and control order unchanged. Acceptance: unit tests pin choice uniqueness, numeric/range targets and type-dependent clearing; focused habit tests and full validation pass.

  ARCH-01 remains open: `WorkoutEditor.kt` still couples compact-entry recovery and timer behavior; `TrackerScreen.kt` still owns tracker-list state; `EditorViewModel.kt` still owns heterogeneous editor persistence. Do not combine these remaining seams into a rewrite.
