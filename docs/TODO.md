...
- [ ] **STYLE-01:** Fix immediate palette refresh after editing/restoring; check contrast and other Material motion. Extend custom palette controls if useful.
  ...
  Status: **IN PROGRESS**. Owner: GitHub Copilot session `33304680-6464-408c-b019-2abe27a4f884`. Branch: `pmathie-cicpilot-persist-workout-drafts`. Claimed: 22 September 2026.

  Acceptance plan: make built-in/custom palette edits and structured preference restores update the active semantic palette without reopening the app; validate editable accent/background/surface combinations against WCAG contrast and prevent unreadable application; propagate reduced-motion preference through the theme so ripple and app-owned animated transitions are disabled or shortened consistently. Add focused preference/palette tests plus emulator interaction evidence for immediate refresh, validation feedback and reduced-motion state. Preserve the existing three custom colour controls unless evidence establishes another semantic token must be directly editable.
