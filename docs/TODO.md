...
- [ ] **ARCH-01:** Split dense editors into maintainable components/state holders, centralise validation and remove superseded private screens without changing the agreed navigation.
  ...
  ARCH-01j (tracker persistence boundary): **IN PROGRESS**. Owner: GitHub Copilot session `33304680-6464-408c-b019-2abe27a4f884`. Branch: `pmathie-cicpilot-persist-workout-drafts`. Claimed: 22 September 2026. Scope: move tracker creation, tracker/field/value saves, active-field reordering and daily clearing from heterogeneous `EditorViewModel` persistence into one focused collaborator; retain the ViewModel API, serialized error handling and Room transaction behavior. Acceptance: focused database tests cover default-field creation, archive-aware reordering and daily value clearing; habit UI tests and full validation pass.

  ARCH-01 remains open: `WorkoutEditor.kt` still couples compact-entry recovery and timer behavior; `TrackerScreen.kt` still owns tracker-list state; `EditorViewModel.kt` still owns non-tracker heterogeneous editor persistence. Do not combine these remaining seams into a rewrite.
