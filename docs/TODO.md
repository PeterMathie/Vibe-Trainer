...
- [ ] **ARCH-01:** Split dense editors into maintainable components/state holders, centralise validation and remove superseded private screens without changing the agreed navigation.
  ...
  ARCH-01l (hold timer component): **IN PROGRESS**. Owner: GitHub Copilot session `33304680-6464-408c-b019-2abe27a4f884`. Branch: `pmathie-cicpilot-persist-workout-drafts`. Claimed: 22 September 2026. Scope: extract the per-exercise hold timer into a focused stateful component with an injectable monotonic clock, retaining its start/elapsed/stop labels and compact-draft update callback. Acceptance: a focused Compose test proves start, elapsed display and stopped value; workout logging/draft tests and full validation pass.

  ARCH-01 remains open: `WorkoutEditor.kt` still owns exercise-card orchestration; `TrackerScreen.kt` still owns tracker-list state; `EditorViewModel.kt` still owns non-tracker heterogeneous editor persistence. Do not combine these remaining seams into a rewrite.
