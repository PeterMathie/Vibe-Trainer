...
- [ ] **ARCH-01:** Split dense editors into maintainable components/state holders, centralise validation and remove superseded private screens without changing the agreed navigation.
  ...
  ARCH-01k (compact set-entry state): **IN PROGRESS**. Owner: GitHub Copilot session `33304680-6464-408c-b019-2abe27a4f884`. Branch: `pmathie-cicpilot-persist-workout-drafts`. Claimed: 22 September 2026. Scope: replace parallel compact performance/RPE UI variables and inline submission validation with one pure form while retaining durable draft writes, idempotent submission, labels and haptics. Acceptance: focused unit tests pin weighted/hold parsing and RPE rejection; workout logging/draft tests and full validation pass.

  ARCH-01 remains open: `WorkoutEditor.kt` still couples exercise-card state and timer behavior; `TrackerScreen.kt` still owns tracker-list state; `EditorViewModel.kt` still owns non-tracker heterogeneous editor persistence. Do not combine these remaining seams into a rewrite.
