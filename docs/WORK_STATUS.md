...
Current implementation task: **UX-23 simplified habits and representative body photos is complete** at `272f902` on `pmathie-cicpilot-persist-workout-drafts`. Habits always record against today and show their colour beside the card title. New measurements use Yes/no, Number with a user-set unit, Written note or Choose from a list; Count, Duration, Rating and Date/time are no longer separate choices. Ordered list values map from light to dark, Yes/no maps No light and Yes dark, and written entries use medium.

Demo data now adds Mood, Journal and Reading with annual non-numeric values plus four generated progress-image placeholders linked to bodyweight dates. Body renders the photo gallery before measurement history. The plate calculator is removed; reduced-motion help remains behind `(i)`. Unit tests and debug app/test assembly passed; focused API 35 Habits (1), Progress (2) and Measurements (1) suites passed. Exact APK SHA-256 is `85034bb7814f57e77c8bd375f1259449f51975ee5d5a9bf281099ea7f2663358`; installed evidence is `files/ux-23-habits.png` and `files/ux-23-body.png`.
