...
Current implementation task: **UX-25 visual habit choice scales and icons is complete** at `5ce5fb2` on `pmathie-cicpilot-persist-workout-drafts`. Choose-from-list configuration now uses draggable named rows ordered from lightest to darkest, previews each row in the habit colour, and exposes explicit last-light and first-dark selectors. Progress classifies choice heat maps from the persisted boundaries. The current seeded Mood scale maps Dejected/Flat/Displeased to light, Agitated/Neutral/Content to medium, and Alert/Happy/Excited to dark.

Habit cards now render a persisted icon from a reusable catalogue and use a compact accessible pencil instead of the full-width settings button. Room schema 11 and migration 10→11 add icon and choice-boundary columns without replacing stored trackers or fields. Follow-up `bc1c5fa` applies the final nine-state Mood vocabulary and upgrades existing debug demo data through seed version 9. Unit tests, debug assembly and focused API 35 Progress (2) passed for that follow-up; the exact APK SHA-256 is `3cbd53d19309858b247f9cd807e892d056a90d360fa74e61c2c253a239faaa61`. The original UX-25 Habits (2), Progress (2) and 10→11 migration gate also passed; installed evidence is under `files/ux-25-habit-choice/`.
