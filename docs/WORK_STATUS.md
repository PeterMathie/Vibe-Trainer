...
Current implementation task: **UX-12 correct programme start and structured set logging is claimed** on `pmathie-cicpilot-persist-workout-drafts`. The bounded plan is to route each programme-list Play action directly into a workout created from that exact programme/day; remove maintained demo weekday prefixes without mutating user-authored or historical names; and reshape active logging into the programmed number of Resistance/Reps/RPE rows with header-level rest timing, notes below and a plus-only extra-set action. Hold/unilateral semantics, durable entry drafts and duplicate prevention remain acceptance constraints.
