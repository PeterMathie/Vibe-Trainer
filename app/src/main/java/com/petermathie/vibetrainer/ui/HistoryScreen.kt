...
fun HistoryScreen(vm:EditorViewModel,onMap:(Long)->Unit) {
    ...
    if(selected!=null) { Column { VibeActionButton("Back to history", { selected=null }, importance = ActionImportance.COMPACT);Box(Modifier.weight(1f)){WorkoutEditor(vm,selected,{}, {selected=null})} };return }
    LazyColumn(Modifier.fillMaxSize(),contentPadding=PaddingValues(16.dp)) {
        item { Text("History",style=MaterialTheme.typography.headlineSmall);EditField("Search workouts and notes",query){query=it};EditField("Body map date (YYYY-MM-DD)",date){date=it};VibeActionButton("View historical body map", { runCatching { LocalDate.parse(date).toEpochDay() }.getOrNull()?.let(onMap) }, importance = ActionImportance.SECONDARY) }
        items(workouts.filter { workout -> workout.status=="FINISHED"&&(workout.name.contains(query,true)||exerciseRows.any { row -> row.workoutId==workout.id && (row.notes.contains(query,true)||row.exerciseName.contains(query,true)||exercises.any { it.id==row.actualExerciseId && it.canonicalName.contains(query,true) }) }) },key={it.id}) { w ->
            Card(Modifier.fillMaxWidth().padding(vertical=5.dp)){Column(Modifier.padding(12.dp)) {
                ...
                Row {VibeActionButton("View / edit", { selected=w.id }, importance = ActionImportance.COMPACT);VibeActionButton("Body map", { onMap(Instant.ofEpochMilli(w.finishedAt ?: w.startedAt).atZone(ZoneId.systemDefault()).toLocalDate().toEpochDay()) }, importance = ActionImportance.COMPACT);VibeActionButton("Remove", { vm.save(w.copy(status="DISCARDED")) }, importance = ActionImportance.COMPACT)}
            }}
        }
    }
}
